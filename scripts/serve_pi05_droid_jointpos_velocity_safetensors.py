...
``--checkpoint-dir`` is required (no default, so the wrong model is never served by accident). Any directory that contains
...
directory that contains ``params/`` (e.g. the official base ``gs://openpi-assets/checkpoints/pi05_droid_jointpos``)
is loaded through the JAX branch. The openpi loader itself is not modified, only the checkpoint directory changes.
"""
...
class Args:
    # Required: directory with model.safetensors (PyTorch) or params/ (JAX), or a gs:// URL.
    checkpoint_dir: str
