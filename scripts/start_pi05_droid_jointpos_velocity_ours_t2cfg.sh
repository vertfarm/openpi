#!/usr/bin/env bash
# Start the KETI DROID velocity server on a team1 PyTorch (safetensors) checkpoint instead of base pi0.5.
#
# Usage:
#   scripts/start_pi05_droid_jointpos_velocity_ours_t2cfg.sh [GPU_INDEX] [CHECKPOINT]
#
#   GPU_INDEX   physical GPU 2 or 3 (default 3)
#   CHECKPOINT  checkpoint directory, or a bare name under /data/keti/snu/workspace/ckpt/team1
#               (default hard5_ours_t2cfg_s20_pytorch). Also settable via CHECKPOINT_DIR=...
#
# Examples:
#   scripts/start_pi05_droid_jointpos_velocity_ours_t2cfg.sh 3
#   scripts/start_pi05_droid_jointpos_velocity_ours_t2cfg.sh 3 hard5_base_ppo_s80_pytorch
#   scripts/start_pi05_droid_jointpos_velocity_ours_t2cfg.sh 3 hard5_opsd_base_s80_pytorch
#
# Loading follows openpi_bhl/docs/260914_checkpoint_loading.md: a directory containing model.safetensors is
# picked up by the upstream PyTorch branch of policy_config.create_trained_policy; no openpi loader code changes.
set -euo pipefail

REPO="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
GPU_INDEX="${1:-3}"
PORT="${PORT:-8000}"
TEAM1_CKPT_ROOT="${TEAM1_CKPT_ROOT:-/data/keti/snu/workspace/ckpt/team1}"
CHECKPOINT_DIR="${2:-${CHECKPOINT_DIR:-hard5_ours_t2cfg_s20_pytorch}}"
...
# Resolve a bare checkpoint name to the team1 checkpoint root; keep gs:// URLs and explicit paths as-is.
if [[ "$CHECKPOINT_DIR" != *"://"* && "$CHECKPOINT_DIR" != */* ]]; then
  CHECKPOINT_DIR="$TEAM1_CKPT_ROOT/$CHECKPOINT_DIR"
fi
...
if [[ "$CHECKPOINT_DIR" != *"://"* ]]; then
  if [[ ! -d "$CHECKPOINT_DIR" ]]; then
    ...
    printf 'available under %s:\n' "$TEAM1_CKPT_ROOT" >&2
    ls -1 "$TEAM1_CKPT_ROOT" >&2 || true
    exit 1
  fi
