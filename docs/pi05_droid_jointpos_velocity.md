...
- A6000 server: physical GPU 3 by default, port 8000
...
checksums, loads the model, and performs two offline warmup inferences before opening port 8000.

Check the running server without constructing `RobotEnv`:

```bash
.venv/bin/python scripts/check_pi05_droid_jointpos_velocity.py --host 127.0.0.1 --port 8000
```
...
Before motion, run the 3090 no-send probe with port 8000 and horizon 15. It must report a finite
