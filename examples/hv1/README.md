...
| 할 일 | 실행 모듈 | 상세 절차 |
|---|---|---|
| 9/10 TODAY30·ALL59 독립 학습 | `examples.hv1.two_track_run` | [두 트랙 학습·최신 ROS 계약](TWO_TRACK_20260910.md) |
| 세션 기록·검수·분할·export | `examples.hv1.readapt` | [재수집·N/M 운영](READAPT_20260910.md) |
| 기준 평가 → smoke → N/M 학습·평가 | `examples.hv1.readapt_run` | 같은 문서의 순차 실행 |
| 개별 학습 / 평가·후보 등록 | `examples.hv1.readapt_train` / `readapt_eval` | 복구·개별 검증용 |
...
| 시연 선정·사람 검수·분할·샘플링 / N·M 레시피 | `readapt.py` — `recipe()`가 학습 설정의 기준 |
| OpenPI 설정 / 명시적 로컬 dataset 로딩 | `readapt_config.py` |
| optimizer 실행·재시작 cursor | `readapt_train.py` |
| 오프라인 지표·SHADOW_ONLY 등록 | `readapt_eval.py` |
| 순차 실행·완료 상태 보관 정책 | `readapt_run.py` |
...
재학습 설정·평가는 trainer를 import하지 않는다. 재학습은 과거 `overnight_*`에도 의존하지 않는다.
...
.venv/bin/python -B -m examples.hv1.readapt --help
```
...
- `overnight*`: [A~F 올나이트 실험 재현](OVERNIGHT_20260909.md), 기존 C/F 후보 설정 및 평가.
- `workflow/cli/review/export/verify_export/demo/adapter`: profile 기반 일반 검수·합성 회귀 테스트.
  실제 KETI HDF5+외부 영상 수집은 위 `readapt/native` 경로를 사용한다.
