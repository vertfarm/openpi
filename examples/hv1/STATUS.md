...
8. **파지 순간 팔은 정지해 있다.** 61개 에피소드에서 측정한 결과, 조작자는 팔을
...
네 번 모두 `gripper_event` 0건, fault 없음, 로봇 명령 0건, 추론률 9.98Hz.
...
1. **모델이 파지 순간 팔을 제대로 세우는가.** 닫기 지연 자체의 위험은 확정 사실 8로
...
7. 제한 실기 rollout — 파지 직전·직후 팔 속도를 기록해 확정 사실 8의 시연 분포와
